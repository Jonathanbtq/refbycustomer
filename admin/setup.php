...
/**
 * \file    refbycustomer/admin/setup.php
 * \ingroup refbycustomer
 * \brief   Refbycustomer setup page.
 */
...
require_once '../lib/refbycustomer.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "refbycustomer@refbycustomer"));

// Initialize a technical object to manage hooks of page. Note that conf->hooks_modules contains an array of hook context
$hookmanager->initHooks(array('refbycustomersetup', 'globalsetup'));
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/refbycustomer/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=refbycustomer-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'REFBYCUSTOMER_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'REFBYCUSTOMER_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "RefbycustomerSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-refbycustomer page-admin');
...
$head = refbycustomerAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "refbycustomer@refbycustomer");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("RefbycustomerSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'REFBYCUSTOMER_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'REFBYCUSTOMER_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
